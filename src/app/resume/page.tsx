        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Build a clean, professional resume. Skills and objective auto-populate from your Nexus profile.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className={`space-y-6 ${showPreview ? 'hidden lg:block' : ''}`}>
              ...
              {/* Objective */}
              <section className="rounded-xl border border-border bg-card p-5">
                <h2 className="font-semibold mb-2 text-sm">Objective</h2>
              ...
              {/* Skills */}
              <section className="rounded-xl border border-border bg-card p-5">
                <h2 className="font-semibold mb-2 text-sm">Skills</h2>
                <p className="text-xs text-muted-foreground mb-3">Pulled from your identity dimensions. Add more below.</p>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {resume.skills.map((skill) => (
                    <Badge key={skill} variant="secondary" className="bg-indigo-600/10 text-indigo-400 border-indigo-600/20 text-xs pr-1">
                      {skill}
                      <button onClick={() => removeSkill(skill)} className="ml-1.5 hover:text-red-400">
                        <X className="size-3" />
                      </button>
                    </Badge>
                  ))}
              ...
              <section className="rounded-xl border border-border bg-card p-5">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="font-semibold text-sm">Projects & Activities</h2>
                  ...
                  {resume.projects.map((project, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      {resume.projects.length > 1 && (
                        <button onClick={() => removeProject(i)} className="text-muted-foreground hover:text-red-400 mt-1">
                  ...
                  {resume.achievements.map((ach, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      {resume.achievements.length > 1 && (
                        <button onClick={() => removeAchievement(i)} className="text-muted-foreground hover:text-red-400">
                          <Trash2 className="size-3.5" />
                        </button>
                      )}
                    </div>
              ...
              {/* Interests */}
              <section className="rounded-xl border border-border bg-card p-5">
                <h2 className="font-semibold mb-3 text-sm">Interests</h2>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {resume.interests.map((interest) => (
                    <Badge key={interest} variant="outline" className="text-xs pr-1">
                      {interest}
                      <button onClick={() => removeInterest(interest)} className="ml-1.5 hover:text-red-400">
                        <X className="size-3" />
                      </button>
                    </Badge>
                  ))}
                ...
                >
                  ...
                  <div className="border-b-2 border-indigo-600 pb-4 mb-4">
                    ...
                    <div className="flex items-center gap-1 mt-2">
                      <GraduationCap className="size-3 text-indigo-600" />
                      <span className="text-[10px] text-indigo-600 font-medium tracking-wide">BUILT WITH NEXUS</span>
                    </div>
                  </div>
                  ...
                  {resume.objective && (
                    <section className="mb-4">
                      <h2 className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">Objective</h2>
                  ...
                  {resume.skills.length > 0 && (
                    <section className="mb-4">
                      <h2 className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">Skills</h2>
                      <div className="flex flex-wrap gap-1">
                        {resume.skills.map((skill) => (
                          <span key={skill} className="inline-block bg-indigo-50 text-indigo-700 text-[10px] px-2 py-0.5 rounded">
                            {skill}
                          </span>
                        ))}
                      </div>
                  ...
                  {resume.projects.some((p) => p.title) && (
                    <section className="mb-4">
                      <h2 className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">Projects & Activities</h2>
                      <div className="space-y-2">
                  ...
                  {resume.achievements.some((a) => a) && (
                    <section className="mb-4">
                      <h2 className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">Achievements</h2>
                  ...
                  {resume.interests.length > 0 && (
                    <section>
                      <h2 className="text-xs font-bold uppercase tracking-wider text-indigo-600 mb-1">Interests</h2>
